...
inline void cvl_rgb_to_hsl(cvl_pixel_t rgb, double *H, double *S, double *L)
{
    ...
    double min = cvl_min3(r, g, b);
    double max = cvl_max3(r, g, b);
...
static inline int cvl_hsl_to_rgb_helper(double tmp2, double tmp1, double H)
{
    double ret;
    ...
    {
	ret = (tmp2 + (tmp1 - tmp2) * ((2.0 / 3.0) - H) * 6.0);
    }
    else
    {   
	ret = tmp2;
    }
    return cvl_iround(255.0 * ret);
}
...
inline cvl_pixel_t cvl_hsl_to_rgb(double H, double S, double L)
{
	...
	tmp2 = 2.0 * L - tmp1;
	r = cvl_hsl_to_rgb_helper(tmp2, tmp1, H + (1.0 / 3.0));
	g = cvl_hsl_to_rgb_helper(tmp2, tmp1, H);
	b = cvl_hsl_to_rgb_helper(tmp2, tmp1, H - (1.0 / 3.0));
    }
    return cvl_pixel_rgb(r, g, b);
}

/* Helpers for sRGB -> CIE L*a*b* conversion */
static inline double cvl_srgb_to_cielab_g(double K)
{
    const double a = 0.055;
    const double gamma = 2.4;
    return (K > 0.04045) ? pow((K + a) / (1 + a), gamma) : (K / 12.92);    
}
static inline double cvl_srgb_to_cielab_f(double t)
{
...
inline void cvl_srgb_to_cielab(cvl_pixel_t srgb, double *L, double *a, double *b)
{
    ...
    const double Zn = 1.0 - Xn - Yn;

    double gR = cvl_srgb_to_cielab_g((double)cvl_pixel_rgb_to_r(srgb) / 255.0);
    double gG = cvl_srgb_to_cielab_g((double)cvl_pixel_rgb_to_g(srgb) / 255.0);
    double gB = cvl_srgb_to_cielab_g((double)cvl_pixel_rgb_to_b(srgb) / 255.0);
    double fXXn = cvl_srgb_to_cielab_f((0.412424 * gR + 0.357579 * gG + 0.180464 * gB) / Xn);
    double fYYn = cvl_srgb_to_cielab_f((0.212656 * gR + 0.715158 * gG + 0.072186 * gB) / Yn);
    double fZZn = cvl_srgb_to_cielab_f((0.019332 * gR + 0.119193 * gG + 0.950444 * gB) / Zn);
    *L = 116.0 * fYYn - 16.0;
    *a = 500.0 * (fXXn - fYYn);
    *b = 200.0 * (fYYn - fZZn);
}
	...
	double hue, double saturation, double lightness, double contrast)
{
    ...
    bool do_hue = (fabs(hue) > 0.0001);
    bool do_saturation = (fabs(saturation) > 0.0001);
    bool do_lightness = (fabs(lightness) > 0.0001);
    bool do_contrast = (fabs(contrast) > 0.0001);
...
void cvl_frame_gamma_correct(cvl_frame_t *frame, double gamma)
{
	...
	for (int i = 0; i < 256; i++)
	{
	    ...
	    {
		table[i] = 16 + cvl_iround(219.0 * pow((double)(i - 16) / 219.0, exponent));
	    }
	}
    }
    else
	...
	for (int i = 0; i < 256; i++)
	{
	    table[i] = cvl_iround(255.0 * pow((double)i / 255.0, exponent));
	}
    }
...
void cvl_frame_gamma_correct_rgb(cvl_frame_t *frame, double gamma_r, double gamma_g, double gamma_b)
{
    ...
    cvl_assert(gamma_g > 0.0);
    cvl_assert(gamma_b > 0.0);

    double exponent_r = 1.0 / gamma_r;
    double exponent_g = 1.0 / gamma_g;
    double exponent_b = 1.0 / gamma_b;
    uint8_t table_r[256];
    uint8_t table_g[256];
    uint8_t table_b[256];
    ...
    for (int i = 0; i < 256; i++)
    {
	table_r[i] = cvl_iround(255.0 * pow((double)i / 255.0, exponent_r));
	table_g[i] = cvl_iround(255.0 * pow((double)i / 255.0, exponent_g));
	table_b[i] = cvl_iround(255.0 * pow((double)i / 255.0, exponent_b));
    }
 ...
 * \param rel_hist	Array of 256 doubles to store the relative histogram in.
 *
...
void cvl_frame_get_rel_histogram(cvl_frame_t *frame, int *abs_hist, double *rel_hist)
{
    for (int i = 0; i < 256; i++)
    {
	rel_hist[i] = (double)abs_hist[i] / (double)(cvl_frame_width(frame) * cvl_frame_height(frame));
    }
}
...
void cvl_frame_equalize_histogram(cvl_frame_t *frame)
{
	...
	for (int i = 0; i < 256; i++)
	{
    	    rpl_tab[i] = cvl_iround(255.0 * (double)sum[i] / (double)(cvl_frame_width(frame) * cvl_frame_height(frame)));
	}
	for (int i = 0; i < cvl_frame_width(frame) * cvl_frame_height(frame); i++)
	{
	...
	for (int i = 0; i < 220; i++)
	{
	    rpl_tab[i] = cvl_iround(219.0 * (double)sum[i + 16] / (double)(cvl_frame_width(frame) * cvl_frame_height(frame)));
	}
	for (int i = 0; i < cvl_frame_width(frame) * cvl_frame_height(frame); i++)
	{
...
int cvl_frame_binarize_get_threshold_otsu(cvl_frame_t *frame)
{
	...
	for (int i = 1; i <= s; i++)
	{
	    m1 += rel_hist[i] * (double)i;
	}
	double m = m1;
	m1 /= p1;
	for (int i = s + 1; i < 256; i++)
	{
	    m += rel_hist[i] * (double)i;
	}
	double sqb = (p1 / (1.0 - p1)) * (m - m1) * (m - m1);
	if (sqb > sqb_max)
	{
	    sqb_max = sqb;
	    s_opt = s;
...
void cvl_frame_binarize_local_threshold(cvl_frame_t *frame, cvl_binarize_local_threshold_type_t type, int k, int C)
{
	...
	for (int y = 0; y < cvl_frame_height(frame); y++)
	{
		...
		T = cvl_iround((double)sum / (double)((2 * k + 1) * (2 * k + 1)));
	...
	for (int y = 0; y < cvl_frame_height(frame); y++)
	{
		...
		{
		    T = cvl_iround((double)(matrix[0] + matrix[matrix_len - 1]) / 2.0);
		}
		cvl_pixel_t p = cvl_frame_get(tmpframe, x, y);
		cvl_frame_set(frame, x, y, (int)p < T - C ? 0x00 : 0xff);
	    }
