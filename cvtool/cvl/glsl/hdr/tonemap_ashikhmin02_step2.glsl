...
float F(float Lwa)
{
    return (C(Lwa) - C(min_abs_lum)) / (C(max_abs_lum) - C(min_abs_lum));
}
...
void main()
{
    ...
    float v_0 = (old_Y - g_1) / old_Y;
    float v_1 = (g_0 - g_1) / g_0;
    float v_2 = (g_1 - g_2) / g_1;
    float v_3 = (g_2 - g_3) / g_2;
    float Lwa = (v_3 <= t) ? g_3 : (v_2 <= t) ? g_2 : (v_1 <= t) ? g_1 : (v_0 <= t) ? g_0 : old_Y;
    float new_Y = clamp(F(Lwa) * old_Y / Lwa, 0.0, 1.0);
