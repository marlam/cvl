...
void cmd_mean_print_help(void)
{
    mh_msg_fmt_req(
	    "mean [-3|--3d] -k|--k=<k>\n"
	    "mean [-3|--3d] -x|--k-x=<kx> -y|--k-y=<ky> [-t|--k-t=<kt>]\n"
	    "\n"
...
int cmd_mean(int argc, char *argv[])
{
	...
	for (;;)
	{
	    ...
	    cvl_mean3d(new_frame, framebuf, kx.value, ky.value, kt.value);
    	...
    	while (!cvl_error())
	{
	    ...
	    cvl_mean(new_frame, frame, kx.value, ky.value);
